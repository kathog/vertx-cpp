...
void Configuration::log(easyloggingpp::base::type::ostream_t& os) const {
...
base::type::string_t DefaultLogBuilder::build(const LogMessage* logMessage, bool appendNewLine) const {
  ...
  easyloggingpp::base::threading::ScopedLock lock_(ELPP->customFormatSpecifiersLock());
  ELPP_UNUSED(lock_);
...
void Writer::triggerDispatch(void) {
  ...
  if (m_proceed && m_level == Level::Fatal
    ...
    reasonStream << "Fatal log at [" << m_file << ":" << m_line << "]"
                 ...
                 << "easyloggingpp::Loggers::addFlag(easyloggingpp::LoggingFlag::DisableApplicationAbortOnFatalLog)";
...
PerformanceTracker::PerformanceTracker(const std::string& blockName,
  ...
  easyloggingpp::Logger* loggerPtr = ELPP->registeredLoggers()->get(loggerId, false);
...
static std::string crashReason(int sig) {
  ...
  for (int i = 0; i < base::consts::kCrashSignalsCount; ++i) {
    if (base::consts::kCrashSignals[i].numb == sig) {
      ...
      if (ELPP->hasFlag(easyloggingpp::LoggingFlag::LogDetailedCrashReason)) {
        ss << std::endl <<
...
#endif  // ELPP_STACKTRACE
  ELPP_WRITE_LOG(easyloggingpp::base::Writer, level, base::DispatchAction::NormalLog, logger) << ss.str();
}
...
void Helpers::crashAbort(int sig, const char* sourceFile, unsigned int long line) {
  ...
  ss << " - [Called easyloggingpp::Helpers::crashAbort(" << sig << ")]";
...
void Helpers::logCrashReason(int sig, bool stackTraceIfAvailable, Level level, const char* logger) {
  easyloggingpp::base::debug::logCrashReason(sig, stackTraceIfAvailable, level, logger);
}
...
void Loggers::setDefaultLogBuilder(easyloggingpp::LogBuilderPtr& logBuilderPtr) {
